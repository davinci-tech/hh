package com.google.android.gms.wearable.internal;

import android.os.IInterface;
import android.os.RemoteException;
import com.google.android.gms.common.api.Status;
import com.google.android.gms.common.data.DataHolder;

/* loaded from: classes2.dex */
public interface zzek extends IInterface {
    void zza(Status status) throws RemoteException;

    void zza(zzbn zzbnVar) throws RemoteException;

    void zza(zzbp zzbpVar) throws RemoteException;

    void zza(zzbt zzbtVar) throws RemoteException;

    void zza(zzdg zzdgVar) throws RemoteException;

    void zza(zzdi zzdiVar) throws RemoteException;

    void zza(zzdk zzdkVar) throws RemoteException;

    void zza(zzdm zzdmVar) throws RemoteException;

    void zza(zzdo zzdoVar) throws RemoteException;

    void zza(zzdr zzdrVar) throws RemoteException;

    void zza(zzdt zzdtVar) throws RemoteException;

    void zza(zzdv zzdvVar) throws RemoteException;

    void zza(zzdw zzdwVar) throws RemoteException;

    void zza(zzdy zzdyVar) throws RemoteException;

    void zza(zzea zzeaVar) throws RemoteException;

    void zza(zzec zzecVar) throws RemoteException;

    void zza(zzee zzeeVar) throws RemoteException;

    void zza(zzeg zzegVar) throws RemoteException;

    void zza(zzf zzfVar) throws RemoteException;

    void zza(zzfq zzfqVar) throws RemoteException;

    void zza(zzfu zzfuVar) throws RemoteException;

    void zza(zzfy zzfyVar) throws RemoteException;

    void zza(zzga zzgaVar) throws RemoteException;

    void zza(zzge zzgeVar) throws RemoteException;

    void zzb(DataHolder dataHolder) throws RemoteException;

    void zzb(zzbt zzbtVar) throws RemoteException;
}
