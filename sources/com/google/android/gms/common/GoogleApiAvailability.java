...
public class GoogleApiAvailability extends GoogleApiAvailabilityLight {
    ...
    public Dialog getErrorDialog(Activity activity, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        return zaa(activity, i, DialogRedirect.getInstance(activity, getErrorResolutionIntent(activity, i, FitRunPlayAudio.PLAY_TYPE_D), i2), onCancelListener);
    }
    ...
    public final boolean zaa(Activity activity, LifecycleFragment lifecycleFragment, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        Dialog zaa2 = zaa(activity, i, DialogRedirect.getInstance(lifecycleFragment, getErrorResolutionIntent(activity, i, FitRunPlayAudio.PLAY_TYPE_D), 2), onCancelListener);
        if (zaa2 == null) {
            return false;
        }
    ...
    private final void zaa(Context context, int i, String str, PendingIntent pendingIntent) {
        ...
        if (DeviceProperties.isWearable(context)) {
            ...
            if (DeviceProperties.isWearableWithoutPlayStore(context)) {
                style.addAction(com.huawei.health.R.drawable._2131427806_res_0x7f0b01de, resources.getString(com.huawei.health.R.string._2130850806_res_0x7f0233f6), pendingIntent);
            } else {
                style.setContentIntent(pendingIntent);
            }
        } else {
            style.setSmallIcon(R.drawable.stat_sys_warning).setTicker(resources.getString(com.huawei.health.R.string._2130850798_res_0x7f0233ee)).setWhen(System.currentTimeMillis()).setContentIntent(pendingIntent).setContentText(errorNotificationMessage);
        }
        if (PlatformVersion.isAtLeastO()) {
