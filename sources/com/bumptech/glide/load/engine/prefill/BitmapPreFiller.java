...
public final class BitmapPreFiller {
    ...
    PreFillQueue generateAllocationOrder(PreFillType... preFillTypeArr) {
        long maxSize = this.memoryCache.getMaxSize();
        long currentSize = this.memoryCache.getCurrentSize();
        long maxSize2 = this.bitmapPool.getMaxSize();
        int i = 0;
        for (PreFillType preFillType : preFillTypeArr) {
            i += preFillType.getWeight();
        }
        float f = ((maxSize - currentSize) + maxSize2) / i;
