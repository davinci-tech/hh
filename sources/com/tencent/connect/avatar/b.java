...
public class b extends View {
    ...
    public Rect a() {
        if (this.f11293a == null) {
            this.f11293a = new Rect();
            int measuredWidth = getMeasuredWidth();
            int measuredHeight = getMeasuredHeight();
            int min = Math.min(Math.min(measuredHeight - 140, measuredWidth), GlMapUtil.DEVICE_DISPLAY_DPI_XXHIGH);
            int i = (measuredWidth - min) / 2;
            int i2 = (measuredHeight - min) / 2;
            this.f11293a.set(i, i2, i + min, min + i2);
        }
        return this.f11293a;
    }
}
