...
public class DeleteDeviceFromCloudSdk implements ProfileRequestInterface {
    private static final String TAG = "DeleteDeviceFromCloud";
    private String baseUrl = CloudSettings.getWiseDeviceUrl() + "/wisedevice/device-registry/v2/devices?devId=";
    ...
    public void request(Context context, ProfileRequestInputParams profileRequestInputParams, ProfileRequestCallback profileRequestCallback) {
        ...
        try {
            ...
            ProfileHttpClient requestBodyCallback = new ProfileHttpClient(context).setUrl(prepareRequestUrl(profileRequestInputParams)).setRequestMethod(ProfileRequestConstants.DELETE_TYPE).setSuccessCode(204).setRequestBodyCallback(new ProfileJsonResponseCallback(TAG, profileRequestCallback));
            ...
            DsLog.et(TAG, "Failed to delete device from cloud, status code = " + e.getErrorType() + " message = " + e.getMessage(), new Object[0]);
