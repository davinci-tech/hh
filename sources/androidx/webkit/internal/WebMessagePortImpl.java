...
public class WebMessagePortImpl extends WebMessagePortCompat {
    ...
    public WebMessagePortImpl(InvocationHandler invocationHandler) {
        this.mBoundaryInterface = (WebMessagePortBoundaryInterface) uxe.b(WebMessagePortBoundaryInterface.class, invocationHandler);
    }
    ...
    private WebMessagePortBoundaryInterface getBoundaryInterface() {
        if (this.mBoundaryInterface == null) {
            this.mBoundaryInterface = (WebMessagePortBoundaryInterface) uxe.b(WebMessagePortBoundaryInterface.class, WebViewGlueCommunicator.getCompatConverter().convertWebMessagePort(this.mFrameworksImpl));
        }
    ...
    public void postMessage(WebMessageCompat webMessageCompat) {
        ...
        if (m.isSupportedByFramework() && webMessageCompat.getType() == 0) {
            ...
            if (m.isSupportedByWebView() && WebMessageAdapter.isMessagePayloadTypeSupportedByWebView(webMessageCompat.getType())) {
                getBoundaryInterface().postMessage(uxe.c(new WebMessageAdapter(webMessageCompat)));
                return;
            }
    ...
    public void setWebMessageCallback(WebMessagePortCompat.WebMessageCallbackCompat webMessageCallbackCompat) {
        ...
        if (m.isSupportedByWebView()) {
            getBoundaryInterface().setWebMessageCallback(uxe.c(new WebMessageCallbackAdapter(webMessageCallbackCompat)));
        } else {
            if (m.isSupportedByFramework()) {
    ...
    public void setWebMessageCallback(Handler handler, WebMessagePortCompat.WebMessageCallbackCompat webMessageCallbackCompat) {
        ...
        if (m.isSupportedByWebView()) {
            getBoundaryInterface().setWebMessageCallback(uxe.c(new WebMessageCallbackAdapter(webMessageCallbackCompat)), handler);
        } else {
            if (m.isSupportedByFramework()) {
