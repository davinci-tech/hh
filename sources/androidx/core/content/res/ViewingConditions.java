...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        float pow2 = (float) Math.pow(((fArr6[0] * cbrt) * f9) / 100.0d, 0.42d);
        float pow3 = (float) Math.pow(((fArr6[1] * cbrt) * f10) / 100.0d, 0.42d);
        float pow4 = (float) Math.pow(((fArr6[2] * cbrt) * f11) / 100.0d, 0.42d);
        float[] fArr7 = {pow2, pow3, pow4};
        float f16 = fArr7[0];
        float f17 = fArr7[1];
        return new ViewingConditions(yFromLStar, ((((f16 * 400.0f) / (f16 + 27.13f)) * 2.0f) + ((f17 * 400.0f) / (f17 + 27.13f)) + (((400.0f * pow4) / (pow4 + 27.13f)) * 0.05f)) * pow, pow, pow, lerp, f12, fArr6, cbrt, (float) Math.pow(cbrt, 0.25d), sqrt + 1.48f);
    }
}
