...
public interface Job extends CoroutineContext.Element {
    ...
    DisposableHandle invokeOnCompletion(Function1<? super Throwable, ueu> handler);

    DisposableHandle invokeOnCompletion(boolean onCancelling, boolean invokeImmediately, Function1<? super Throwable, ueu> handler);
    ...
    Object join(Continuation<? super ueu> continuation);
    ...
    public static final class e {
        @Deprecated(level = DeprecationLevel.ERROR, message = "Operator '+' on two Job objects is meaningless. Job is a coroutine context element and `+` is a set-sum operator for coroutine contexts. The job to the right of `+` just replaces the job the left of `+`.")
        public static Job b(Job job, Job job2) {
            return job2;
        }

        public static <E extends CoroutineContext.Element> E b(Job job, CoroutineContext.Key<E> key) {
            return (E) CoroutineContext.Element.b.d(job, key);
        }

        public static CoroutineContext b(Job job, CoroutineContext coroutineContext) {
            return CoroutineContext.Element.b.b(job, coroutineContext);
        }

        public static <R> R c(Job job, R r, Function2<? super R, ? super CoroutineContext.Element, ? extends R> function2) {
            return (R) CoroutineContext.Element.b.b(job, r, function2);
        }

        public static CoroutineContext c(Job job, CoroutineContext.Key<?> key) {
            return CoroutineContext.Element.b.e(job, key);
        }
