...
public final class RxJavaPlugins {
    ...
    public static <T> Subscriber<? super T>[] onSubscribe(ParallelFlowable<T> parallelFlowable, Subscriber<? super T>[] subscriberArr) {
        BiFunction<? super ParallelFlowable, ? super Subscriber[], ? extends Subscriber[]> biFunction = onParallelSubscribe;
        return biFunction != null ? (Subscriber[]) apply(biFunction, parallelFlowable, subscriberArr) : subscriberArr;
    }
    ...
    public static void setOnParallelSubscribe(BiFunction<? super ParallelFlowable, ? super Subscriber[], ? extends Subscriber[]> biFunction) {
        if (lockdown) {
            throw new IllegalStateException("Plugins can't be changed anymore");
        }
        onParallelSubscribe = biFunction;
    }
    ...
    public static Scheduler createExecutorScheduler(Executor executor, boolean z, boolean z2) {
    ...
    static Scheduler callRequireNonNull(Supplier<Scheduler> supplier) {
        try {
            return (Scheduler) Objects.requireNonNull(supplier.get(), "Scheduler Supplier result can't be null");
        } catch (Throwable th) {
            throw ExceptionHelper.wrapOrThrow(th);
        }
    }
    ...
    static Scheduler applyRequireNonNull(Function<? super Supplier<Scheduler>, ? extends Scheduler> function, Supplier<Scheduler> supplier) {
        return (Scheduler) Objects.requireNonNull(apply(function, supplier), "Scheduler Supplier result can't be null");
    }
